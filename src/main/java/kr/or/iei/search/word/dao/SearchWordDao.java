...
public class SearchWordDao {
	...
	public Boolean searchString(Connection conn, String srchInput) {
		...
		//없으면 컬럼추가 + 양1 || 있으면 양+1
		String query = "MERGE INTO tbl_search sr USING (SELECT ? AS srch_Name FROM dual) new_word ON (sr.srch_Name = new_word.srch_Name) WHEN MATCHED THEN UPDATE SET sr.srch_amount = sr.srch_amount + 1 WHEN NOT MATCHED THEN INSERT (srch_Name, srch_amount) VALUES (new_word.srch_Name, 1)";
